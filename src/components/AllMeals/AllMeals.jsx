...
import { FaMapMarkerAlt, FaStar, FaSortAmountDown, FaRedo } from 'react-icons/fa';
...
const AllMeals = () => {
    ...
    const { data: result, isLoading } = useQuery({
        queryKey: ['allmeals', order, currentPage, searchTerm, selectedArea, selectedRating],
        queryFn: async () => {
    ...
    return (
        <div className="bg-white dark:bg-neutral-950 min-h-screen pb-20 font-sans">
            ...
            <div className="max-w-[1440px] mx-auto px-6 pt-12 pb-6 border-b border-gray-100 dark:border-white/5">
                ...
                <p className="text-sm text-gray-400 mt-1 font-medium">{totalCount} available in your region</p>
            </div>
                    ...
                    <div className="sticky top-24 space-y-10">
                        ...
                        <div>
                            <h3 className="text-xs font-black uppercase tracking-widest text-neutral-900 dark:text-gray-400 mb-5 flex items-center gap-2">
                                <FaMapMarkerAlt className="text-[#f97416]" /> Location
                            </h3>
                            <div className="flex flex-col gap-2">
                                <button 
                                    onClick={() => setSelectedArea("")}
                                    className={`text-left px-4 py-3 rounded-xl text-sm font-bold transition-all ${selectedArea === "" ? "bg-[#f97416] text-white shadow-lg shadow-orange-500/20" : "bg-gray-50 dark:bg-neutral-900 hover:bg-gray-100 text-neutral-600"}`}
                                >
                                    Everywhere
                                </button>
                                {areas.map(area => (
                                    <button 
                                        ...
                                        className={`text-left px-4 py-3 rounded-xl text-sm font-bold transition-all ${selectedArea === area ? "bg-[#f97416] text-white shadow-lg shadow-orange-500/20" : "bg-gray-50 dark:bg-neutral-900 hover:bg-gray-100 text-neutral-600"}`}
                                    >
                                        {area}
                                    </button>
                                ))}
                            ...
                            <div className="space-y-3">
                                {ratings.map(r => (
                                    <label key={r.value} className="flex items-center gap-3 cursor-pointer group">
                                        ...
                                        />
                                        <span className={`text-sm font-bold ${selectedRating === r.value ? "text-neutral-900" : "text-gray-500 group-hover:text-neutral-700"}`}>
                                            {r.label}
                                        </span>
                                    </label>
                                ))}
                        ...
                        <button 
                            onClick={() => {setSelectedArea(""); setSelectedRating(""); setOrder("desc")}}
                ...
                <main className="flex-1">
                    ...
                    <div className="flex justify-end items-center mb-8">
                        <div className="flex items-center gap-2 bg-gray-50 dark:bg-neutral-900 px-4 py-2 rounded-full border border-gray-100 dark:border-white/5 shadow-sm">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-tighter text-gray-400">Sort:</span>
                            <select 
                                value={order}
                                onChange={(e) => setOrder(e.target.value)}
                                className="bg-transparent text-xs font-bold text-neutral-900 dark:text-white focus:outline-none cursor-pointer pr-2"
                            >
                                <option value="desc" className='font-bold'>Price: High to Low</option>
                    ...
                    {isLoading ? (
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
                            {[...Array(8)].map((_, i) => (
                                <div key={i} className="h-72 w-full bg-gray-100 dark:bg-neutral-800 animate-pulse rounded-2xl"></div>
                            ))}
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
                            {meals.length > 0 ? (
                                ...
                                <div className="col-span-full py-32 text-center bg-gray-50 rounded-3xl border border-dashed border-gray-200">
                                    <h3 className="text-xl font-bold text-gray-400 italic">No matches found. Try resetting filters.</h3>
                                </div>
                            )}
                        </div>
                    )}
